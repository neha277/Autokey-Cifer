...
def decrypt(input,op,key_val):
    ...
    for i in input:
        text = alpha[i]
        text -= alpha[key_val[index_count]]
        if text < 0:
            text += 26
        key_val_temp =  [k for k,v in alpha.items() if v == text][0]
        key_val +=  key_val_temp
        text %= 26

        index_count += 1
        plain_text_value.append([k for k,v in alpha.items() if v == text][0])

    # print(''.join(plain_text_value)  )
    with open(op, "w+") as f:
        f.write(''.join(plain_text_value))
        f.truncate()
    # return ''.join(plain_text_value)    
    # for i in input:
    #     plain_text_value.append(alpha[i])
